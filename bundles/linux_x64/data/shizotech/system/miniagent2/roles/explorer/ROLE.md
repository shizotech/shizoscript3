...
* Report what has been explicitly observed via `read_file()`
...
* ONLY call `read_file()` on the specified file(s)
...
* Actively explore the repository using `list_dir()` and `read_file()`
...
(!) Only use research and retrieval tools