...
4. Inspect repository files and directories as needed, read required files with `read_file()` and try to avoid reading large files.
   -> Only read necessary files
   -> Try to avoid reading large files when possible

5. Check if any changes are necessary or if the task appears to already be completed.

   → If no work needs to be done, emit a **FINAL TASK REPORT** with `final_report()` and exit immediately.

6. Define the steps required to complete the task.

7. Apply tools deliberately.  
   Before each tool call, explain what you are about to do.

8. Apply changes via `replace_string()` or create new files with `write_file()` using validated content.
   -> Avoid applying the same patches multiple times
   -> If a patch fails, try different approaches

9. After completing all actions, verify changes and emit a **FINAL TASK REPORT** with `final_report()`.
...
3. If the task creates or modifies a file type that is commonly referenced (e.g. HTML, CSS, JS, config files):
   ...
   - Read relevant files using `read_file()` to understand current structure.
...
1. Prefer `replace_string()` for modifications
2. Prefer the smallest viable change
3. Use `write_file()` only for new files or full overwrites

---

## WHEN TO USE replace_string()

Use `replace_string()` when:
...
Never use `write_file()` for partial modifications.
